		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
		            ...
		            html += '				<div id="status_info" class="info">图片大小0B。</div>';
		            html += '				<div class="btns">';
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSuccess: function(file, response) {
					    ...
					    $('.upload').attr("src","/static/upload/"+response)
					    //$('.theme-login').html('上传成功');
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							$("#uploadList_" + file.index).fadeOut();
